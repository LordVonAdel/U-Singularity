...
objects: { //-----------------------Objects-----------------------------
  door_default: {"image":"objects/door_default.png", "image_number":8, "image_index":0, "collision":true, 
                "sync":{open: 0, frame: 0},
                ...
                "onAnimation":function(ent){ent.sync.frame = handy.transition(ent.sync.frame,ent.sync.open,1/30,0);ent.image_index = Math.floor(ent.sync.frame*(ent.image_number-1)); ent.collision = (Math.floor(ent.sync.frame)==0); ent.update(); ent.share(); if(ent.sync.frame == ent.sync.open){ent.animation = false}},
                "onPush":function(user,ent){ent.sync.open = 1; ent.animation = true},
                "onUpdate":function(ent){ent.image_index = Math.floor(ent.sync.frame*(ent.image_number-1)); ent.collision = (Math.floor(ent.sync.frame)==0);},
                "actions":{},
                "tile":{"connectionGroup":"walls"},
                "layer":30
                },
  detail:       {"image":"detail/exit_signs.png", "image_number":1, "image_index":0, "collision":false,
                "actions":{}
                },
